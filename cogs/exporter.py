...
class Exporter(commands.Cog):
    ...
    @discord.slash_command(name='export', description='EXPORTS MESSAGES.', guild_only=True)
    @commands.has_permissions(read_message_history=True)
    async def export(self, ctx, file_type: Option(str, 'which file type?', choices=['HTML (Dark Mode)', 'HTML (Light Mode)', 'TXT (Plain)'], required=True), start_after: Option(str, 'COPY MESSAGE LINK OR MESSAGE ID', required=False), end_before: Option(str, 'COPY MESSAGE LINK OR MESSAGE ID', required=False)):
        loading_embed = basic_embed_creator('⏳\a Processing messages... Please wait...', None)
        await ctx.respond('_ _', embed=loading_embed)
